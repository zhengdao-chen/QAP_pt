...
from pyGAT_code.models import GAT, SpGAT, GAT_ml
...
parser.add_argument('--folder_name', nargs='?', const=1, type=str, default='haha')
...
if args.sparse:
    ...
    #             alpha=args.alpha,
    #             bn_or_not=args.bn)
    model = GAT_ml(nlay=args.num_layers,
...
def train(model, epoch): #, inputs_batch, labels_batch, W_batch):
    ...
    for i in range(args.batch_size):
        ...
        features = Variable(torch.from_numpy(x_from_W(W[0, :, :])), volatile=False).type(dtype)
        adj = Variable(torch.from_numpy(W[i, :, :]), volatile=False).type(dtype)
        labels = labels[i, :].type(dtype_l)
...
def test_mcd_single(model, gen, n_classes, it):
    ...
    features = Variable(torch.from_numpy(x_from_W(W[0, :, :])), volatile=False).type(dtype)
    adj = Variable(torch.from_numpy(W[0, :, :]), volatile=False).type(dtype)
    labels = labels[0, :].type(dtype_l)
    ...
    info = ['iter', 'loss', 'acc', 'edge_density',
            'noise', 'model', 'elapsed']
    out = [it, loss_value, acc_test, args.edge_density,
           args.noise, 'GAT' + str(args.num_layers), elapsed]
    print(template1.format(*info))
    print(template2.format(*out))

    return loss_value, acc_test

def test(model, gen, n_classes, iters=args.num_examples_test):
    model.train()
    ...
    for it in range(iters):
        ...
        loss_single, acc_single = test_mcd_single(model, gen, n_classes, it)
...
folder_name = str(args.p_SBM) + '_' + str(args.q_SBM)

for epoch in range(args.epochs):
    loss_values.append(train(model, epoch))#, inputs_batch, labels_batch, W_batch))

    # torch.save(model.state_dict(), './' + str(args.p_SBM) + '_' + str(args.q_SBM) + '/' + '{}.pkl'.format(epoch))
    torch.save(model.state_dict(), './' + folder_name + '/' + '{}.pkl'.format(epoch))
...
model.load_state_dict(torch.load('./' + folder_name + '/' +  '{}.pkl'.format(best_epoch)))

# Testing
# compute_test(model)
test(model, gen, args.n_classes, iters=args.num_examples_test)
if args.bn:
    print ('bn2d')
else:
    print ('sub mean')
